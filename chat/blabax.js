...
dbid = 0;
ajnm = 0;
sound = 0; // usr_online[id]=[ext.id,group,name]
dragypos = 0;
dragdown = false;
allowselect = 0;
autoscroll = 1;
...
freeaudio = 1;
usrinteaction = 0;
ignored_users = new Array("0");
pmnotifications = new Array("0");
uxtra_motto = new Array();
uxtra_avatars = {};
uxtra_expire = new Array();
phistorycached = new Array();
phistoryhidebtn = new Array();
srv_usr_id = 0;
ext_usr_id = 0;
active_usrname = "";
avatars_loaded = 0;
div_roomhistory = false;
div_userhistory = false;
blockroomchange = 0;
blockuserchange = 0;
ax_intv = false;
pmarrnotify = [];
...
zone = new Date();
zone = -zone.getTimezoneOffset();

try {
  dtitle = top.document.title.toString();
} catch (e) {
  dtitle = false;
}
if (
...
if (window.parent && typeof mobileapp != "undefined" && mobileapp > 1) {
  function play_s(x) {
    if (sound_on == 1 && x > 0 && sound_options[x] > 0) {
...
function verti_pos() {
  ...
  if (offset > 0) {
    de("middps").style.marginTop = offset + "px";
  }
}
...
function shoop(x, y, z) {
  ...
  setTimeout(
    "obj2unhoop.style.transform='scale(1,1)';obj2unhoop.style.transform='rotate(0deg)'",
    z
  );
}
...
function clear_hoop() {
  if (typeof opad1 == "number") {
    clearTimeout(opad1);
  }
  if (typeof opad2 == "number") {
    clearTimeout(opad2);
  }
  if (obj2hoop) {
    if (hoop_direction == 1) {
...
function hoop(a, b) {
  ...
  if (b == 1 && cdispl(a) == "none") {
    ...
    obj2hoop = a;
    opad1 = setTimeout(
      "obj2hoop.style.opacity=1;obj2hoop.className+=' sfade1'",
      50
    );
...
function init() {
  ...
  // only show online users to the admins - hidden for other participants
  if (myuuid > 5) {
    document.getElementById("radiobutton").style.display = "none";

    document.getElementById("online_users").style.display = "none";
    //console.log('here',document.getElementById('online_users').style.display )
  }
}
...
function ax_ping(msg) {
  ...
  rq =
    ...
    "&tousername=" +
    encodeURIComponent(active_usrname) +
    "&status=" +
    current_status +
    "&ohash=" +
    ohash +
  ...
  ax_pulse.setRequestHeader(
    "Content-Type",
    "application/x-www-form-urlencoded"
  );
  ax_pulse.onreadystatechange = ax_ansr;
  ax_pulse.send(rq);
...
function change_room(x) {
  if (typeof rooms[x] == "object" && x != current_room && blockroomchange < 1) {
    rmb_txt("r_hide");
    current_room = x;
    log.innerHTML = split_room_content(x);
    scrolllog();
    play_s(4);
    ascroll();
    recalc_msg();
...
function split_room_content(x) {
  if (rooms[x][4] <= msgs2keep) {
    return rooms[x][3] + rooms[x][1];
  } else {
...
function key_changer(e) {
  ...
  troom = e - 48;
  if (rcodes[troom]) {
    change_room(rcodes[troom]);
    return;
  }

  if (e == 39) {
    stpp = 0;
    index = rcodes.indexOf(current_room);
    for (i = 1; i < rcodes.length; i++) {
      if (stpp == 1) {
        change_room(rcodes[i]);
        return;
      }
      if (i == index) {
        stpp = 1;
...
function displaysomething() {
  x = document.getElementById("something").style.background = "red";
  console.log(x);
}
...
function msg_format(x) {
  ...
  for (i in stickers) {
    if (stickers[i] == x) {
      msgsticker = 1;
      x = '<img src="' + x + '" class="chat_area_sticker" alt="" />';
    }
  }
  if (msgsticker) {
    return x;
...
function msg_display(x) {
  ...
  htmlmsg = msg_template
    .replace("{AVATAR}", avsrc)
    .replace("{TIME}", time)
    .replace("{GROUP}", "1")
    .replace("{NAME}", name)
    .replace("{UID}", from)
  ...
  if (from < 1 || room > 0) {
    ...
    if (pbox != ext_usr_id) {
      ...
      if (notify_now > 0) {
        ...
        msg2add = msg_template
          .replace("{AVATAR}", "img/000.svg")
          .replace("{TIME}", "")
          .replace("{GROUP}", "1")
          .replace("{NAME}", lang["system"])
          .replace("{UID}", 0)
          .replace("{COLOR}", 0)
          .replace(
            "{TEXT}",
            lang["pmmsg"] +
              ' <b class="pointer x_accent_fg" onclick="show2user(' +
              pbox +
              ',this)">' +
              name +
              "</b>"
        ...
        if (
          de("panel_pmlog").style.display != "block" ||
          pan.style.display != "block"
        ) {
          pmarrnotify_add(from);
          de("top2unread").style.display = "block";
          de("bot2unread").style.opacity = 1.0;
        }
        sound = 3;
      ...
      tcss = tcss
        .replace("pchat_notify_on", "")
        .replace("pchat_notify", "")
        .trim();
...
function pm_reorder(x, y) {
  z = get_time();
  if (uxtra_avatars[x]) {
    pmavatr = atob(uxtra_avatars[x]);
  } else {
    pmavatr = "avatar.php?q=" + x;
  }
  pmele =
    '<div id="pmd' +
    x +
    '" class="x_accent_bb pointer panel_pmitem" onclick="if(!show2user(' +
    x +
    ",this)){setTimeout('manage_esc()',100)}\">";
  pmele += '<img class="panel_pmavtr x_circle" src="' + pmavatr + '" alt="" />';
  pmele +=
    '<div style="font-weight:bold;text-align:left" class="shorten">' +
    y +
    "</div><small>" +
    z +
    '</small><br style="clear:both" /></div>';
  if (de("pmd" + x)) {
    de("pmd" + x).parentNode.removeChild(de("pmd" + x));
  }
  s = de("pmlog_container").innerHTML;
  de("pmlog_container").innerHTML = pmele + s;
}
...
function recalc_msg() {
  u = 0;
  rooms[current_room][2] = 0;
  for (i in rooms) {
    if (typeof rooms[i] == "object") {
      u += rooms[i][2];
      de("unr" + i).innerHTML = rooms[i][2];
    }
  }
  if (u > 0) {
    de("top_unread").style.display = "block";
    de("bot_unread").style.opacity = 1.0;
  } else {
    de("top_unread").style.display = "none";
    de("bot_unread").style.opacity = 0.0;
  }
  ...
  for (i in uo) {
    ...
    for (j in ignored_users) {
      if (uo[i][2] == ignored_users[j]) {
        linethrough =
          ' style="text-decoration:line-through;opacity:0.2;opacity:0.2"';
      }
    }
    ...
    onl_str.push(
      "<!-- " +
        ...
        ",'" +
        uo[i][2] +
        '\')" class="single_online_user shorten pointer"><img src="' +
        atob(uo[i][4]) +
        '" class="x_circle" alt="" /><b class="x_bcolor_x"><i id="e' +
        uo[i][0] +
        '" class="' +
        ntfy +
        '"></i></b><span' +
    ...
    if (!de(pu)) {
      p.id = pu;
      p.className = "one2chat x_bottom_rounded x_bcolor_x svg_pmpm";
      pch.appendChild(p);
    }
    usr_length += 1;
  }
...
function uxtra_init(x) {
  if (uxtra_motto[x]) {
    de("user_avatar").style.backgroundImage =
      "url(" + atob(uxtra_avatars[x]) + ")";
    if (uxtra_motto[x].length < 33) {
      sh_motto = uxtra_motto[x];
    } else {
      sh_motto = uxtra_motto[x].substr(0, 30);
      sh_motto = sh_motto + "...";
    }
...
function ignore_init() {
  ...
  tmp = emute;
  for (i in ignored_users) {
    if (ignored_users[i] == active_usrname) {
      tmp = dmute;
    }
  }
...
function ignore_set() {
  ...
  if (!pss) {
    ignored_users.splice(1, 0, active_usrname);
    p.className = dmute;
  } else {
    ignored_users.splice(pss, 1);
    p.className = emute;
  }
  online_construct();
  localStorage.setItem(storkey_ign, JSON.stringify(ignored_users));
}
...
function ban_user(b) {
  ...
  ajax_bus.setRequestHeader(
    "Content-Type",
    "application/x-www-form-urlencoded"
  );
  ajax_bus.onreadystatechange = ban_ans;
  ajax_bus.send(x);
  if (lof.style.display != "block") {
    hsp.style.display = "none";
  }
  setTimeout("ax_ping(0)", 500);
  hide_user();
}
...
function hidepm_notify() {
  ...
  de("bot2unread").style.opacity = 0.0;
}

function pmarrnotify_add(x) {
  x = parseInt(x);
  if (pmarrnotify.indexOf(x) < 0) {
    pmarrnotify.push(x);
  }
}
...
function pmarrnotify_rem(x) {
  x = parseInt(x);
  if (pmarrnotify.indexOf(x) > -1) {
    pmarrnotify.splice(pmarrnotify.indexOf(x), 1);
  }
}
...
function manage_esc() {
  ...
  if (srv_usr_id != 0) {
    hide_user();
    hsp.style.display = "none";
    if (mobile < 1 && lof.style.display == "block") {
      hoop(lof, 0);
    }
    if (mobile > 0 && lof.style.display == "block") {
      lof.style.display = "none";
    }
    inp_focus();
    return;
...
function manage_wfocus(x) {
  if (mobile < 1) {
    if (x > 0) {
      ...
      de("hidescreen_blur").style.display = "block";
    }
  }
}
...
function sound_opt_init() {
  ...
  for (i = 0; i < 10; i++) {
    acti = "snd" + i + "oo";
    if (de(acti)) {
      if (typeof sound_options[i] != null && sound_options[i] > 0) {
        de(acti).className = stonn;
        co = 1;
      } else {
        de(acti).className = stoff;
        co = 0;
      }
      sound_options[i] = co;
    }
...
function swap_sound(u) {
  x = de("sndd");
  if (sound_on == 1) {
    x.className = "panel_button x_bcolor_z x_all_rounded svg_soff";
    sound_on = 0;
    if (u > 0) {
      settings_save("sound", sound_on);
    }
    return;
  }
  if (sound_on == 0) {
    x.className = "panel_button x_bcolor_z x_all_rounded svg_sndd";
    sound_on = 1;
    if (u > 0) {
      settings_save("sound", sound_on);
    }
...
function swap_color(x, y, u) {
  ...
  inp.style.color = y;
  if (u > 0) {
    settings_save("color", x);
    settings_save("colva", y);
    hst.style.display = "none";
    pab.style.display = "none";
    hoop(pan, 0);
    inp_focus();
  ...
  for (key in emos) {
    if (typeof emos[key] !== "function") {
      x = replace_all(
        x,
        key,
        '<span class="' + emos[key] + ' chat_area_emoticon"></span>'
      );
    }
  }
  return x;
...
function repl_links(x) {
  ...
  x = x.replace(
    pattern,
    '<span style="text-decoration:underline" class="pointer x_accent_fg" onclick="wopen(\'$1\',1)">$1</span>'
  );
  return x;
}
...
function inp_focus() {
  if (mobile < 1 && chatflow < 1 && inp.readOnly == false) {
    inp.focus();
  }
}
...
function user_check_form() {
  ...
  if (f.newpass.value.trim().length > 2) {
    if (f.newpass.value != f.retype.value) {
      f.retype.className = s;
      shoop(de("prosubbutton"), 2, 200);
      return false;
    }
    if (f.oldpass.value.trim().length < 3) {
      f.oldpass.className = s;
      shoop(de("prosubbutton"), 2, 200);
      return false;
    }
  }
  ...
  if (f.email.value.trim() != f.dbmail.value) {
    if (f.email.value.trim().length < 7) {
      f.email.className = s;
      shoop(de("prosubbutton"), 2, 200);
      return false;
    }
    ...
    if (f.answer.value.trim().length < 1) {
      f.answer.className = s;
      shoop(de("prosubbutton"), 2, 200);
      return false;
    }
  }
  ...
  if (
    f.newpass.value.trim().length < 3 &&
    f.email.value.trim() == f.dbmail.value
  ) {
    shoop(de("prosubbutton"), 2, 200);
    return false;
  }
...
function guest_check_form() {
  ...
  if (f.password.value.trim().length < 3) {
    f.password.className = s;
    shoop(de("prosubbutton"), 2, 200);
    return false;
  }
  if (f.password.value != f.retype.value) {
    f.retype.className = s;
    shoop(de("prosubbutton"), 2, 200);
    return false;
  }
  ...
  if (f.question.value.trim().length < 1) {
    f.question.className = s;
    shoop(de("prosubbutton"), 2, 200);
    return false;
  }
  if (f.answer.value.trim().length < 1) {
...
function panput_style_back(x, y) {
  x.className = "x_bcolor_y x_accent_bb panel_input " + y;
}
...
function wmove(e) {
  ...
  log.scrollTop += z;
  if (autoscroll < 1 && rbotto()) {
    hoop(aus, 0);
    autoscroll = 1;
    return;
  }
  if (autoscroll > 0 && !rbotto()) {
    hoop(aus, 1);
    autoscroll = 0;
  }
}
...
function rbotto() {
  if (parseInt(log.scrollHeight - log.scrollTop) <= log.clientHeight) {
    return true;
  } else {
    return false;
  }
...
function rcv_r_history() {
  if (ajax_get_rmsg.readyState == 4) {
    blockroomchange = 0;
    div_roomhistory.innerHTML = "";
  }
...
function show_p_history(x) {
  if (phistorycached.indexOf(ext_usr_id) != -1) {
    return;
  }
  blockuserchange = 1;
...
function rcv_p_history() {
  if (ajax_get_pmsg.readyState == 4) {
    blockuserchange = 0;
    div_userhistory.innerHTML = "";
  }
...
function multimsg_prepare(r) {
  try {
    ...
    for (w in r) {
      ...
      msgft = msg_template
        .replace("{AVATAR}", h_avat)
        .replace("{TIME}", h_time)
        .replace("{GROUP}", "1")
        .replace("{NAME}", h_uname)
        .replace("{UID}", h_usrid)
        .replace("{COLOR}", h_color)
        .replace("{TEXT}", h_text);
      msgfromdb[h_id] = msgft;
    }

    msgfromdb = msgfromdb.join(" ");
    if (msgfromdb.length < 1) {
      msgfromdb = lang["nomsg"];
    }
    return msgfromdb;
  } catch (e) {
    console.log(e);
...
function rmb_txt(w) {
  ...
  if (w == "u_hide") {
    ...
    if (arr_rmb_rooms[current_room]) {
      inp.value = arr_rmb_rooms[current_room];
    } else {
      inp.value = "";
    }
  }
...
function load_hist_init() {
  setTimeout("show_r_history(de('welcome_msg'))", 60);
  setTimeout("scrolllog()", 80);
  setTimeout("scrolllog()", 120);
}
...
function sel_avatar(ax) {
  pan.scrollTop = 0;
  de("my_avatar_pic").className = "";
  de("avupload").value = "";
  de("avatar").value = ax;
  de("my_avatar_pic").src = ax;
  setTimeout(
    "shoop(de('avmottosbutton'),2,200);de('avmottosbutton').className='x_all_rounded x_accent_bg panel_button'",
    500
  );
...
function avformcheck(ai, sz) {
  ...
  if (typeof ai.files[0] == "object" && ai.files[0].size < sz) {
    ...
    ai.value = "";
    de("avatar").value = de("avinit").value;
    tav.src = de("avinit").value;
    ...
    setTimeout(
      "de('lblforup').style.opacity=1;de('avmaxsizedesc').className='x_overal x_right_rounded'",
      950
    );
  }
}
...
if (typingTimerValue == undefined) {
  typingTimerValue = typingTimerValueDefault;
}
if (typingTimerValue > 60000 || typingTimerValue < 100) {
  typingTimerValue = typingTimerValueDefault;
}

function chatKeyPress(event) {
  // Get IsTyping settings
  if (Number(localStorage.getItem("eduTechIsTyping")) == false) {
    return;
  }

  if (event.keyCode != 13) {
    if (typing != true) {
      change_status(6, "Typing");
      sendIsTypingMessage();
    }
    typing = true;
...
function sendIsTypingMessage() {
  // Make an AJAX request to send the "is typing" message to the server
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "typingstatus.php", true);
...
function checkTypingStatus() {
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "gettypingstatus.php", true);
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
      var typingStatusContainer = document.getElementById(
        "typingStatusContainer"
      );
      typingStatusContainer.innerHTML = xhr.responseText;

      // Schedule the next check after a certain interval (e.g., every 5 second)
      setTimeout(checkTypingStatus, 200);
    }
  };
  xhr.send();
}
