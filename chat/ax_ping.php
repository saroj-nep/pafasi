...
if (!isset($_POST['msg']) || !isset($_POST['mtoken']) || !isset($_POST['status']) || !isset($_POST['ohash']) || !isset($_POST['dbid']) || !isset($_POST['ampm']) || !isset($_POST['ajnm']) || !isset($_POST['zone']) || !isset($_POST['tousername'])) {
	print 'error 001';
	die();
}
...
// exit
if ($_POST['msg'] == '1') {
	neutral_query('DELETE FROM ' . $dbss['prfx'] . '_online WHERE id=' . $userid);
...
if ($_POST['msg'] != '0') {
	...
	if (!is_array($message) || !isset($message['to']) || !isset($message['color']) || !isset($message['text']) || !isset($message['room'])) {
		print 'error 006';
		die();
	}

	if (isset($message['attach'])) {
		$attach = (int) $message['attach'];
	} else {
		$attach = 0;
	}

	if ($attach == 4) {
		$message['text'] = rolldice($message['text']);
	}

	$roomid = (int) $message['room'];
	$touser = (int) $message['to'];
	if ($touser > 0) {
		$roomid = 0;
	}
...
if (isset($msg_leave) && isset($sys_xuser)) {
	...
	if (neutral_num_rows($res) > 0) {
		...
		neutral_query('INSERT INTO ' . $dbss['prfx'] . "_messages VALUES(NULL,999999,0,'$usergroup','$sys_xuser',0,'','$usr2tout $msg_leave',0,0,$timestamp)");
	}
}
...
if ($dbid < 1) {
	$res = neutral_query('SELECT MAX(id) FROM ' . $dbss['prfx'] . '_messages');
	$res = neutral_fetch_array($res);
	$dbid = $res[0];
} else {
