...
//$res=neutral_query('SELECT * FROM '.$dbss['prfx'].'_rooms WHERE groupids=' .$group);
$res=neutral_query('SELECT * FROM '.$dbss['prfx'].'_rooms ORDER BY zorder,id ASC');
while($row=neutral_fetch_array($res)){
	//echo("<script>console.log('PHP: " . $row['groupids'] . "');</script>");
	//echo("<script>console.log('user: " . $xuser['ugroup'] . "');</script>");


//if($row['id']==$room_coosel && $room_getsel<1){$landing_room=$room_coosel;}
if($row['groupids']==$group){$landing_room=$row['id'];
