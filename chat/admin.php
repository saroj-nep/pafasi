...
if(isset($_POST['ugroup'])){
	$ugroup=$_POST['ugroup'];
    neutral_query('UPDATE '.$dbss['prfx']."_users SET ugroup='$ugroup' WHERE id=$id");
}
...
neutral_query('INSERT INTO '.$dbss['prfx']."_rooms VALUES(NULL,'NEW ROOM','Description...','$zcolor',1,$zorder,0,1)");
...
if(isset($_POST['groupsid'])){$groupsid=(int)$_POST['groupsid'];}else{$groupsid=1;}
if($zorder<1000){$zorder+=1000;} if($rid==1){$zorder=0;}

neutral_query('UPDATE '.$dbss['prfx']."_rooms SET name='$name',description='$desc',color='$color',onoff=$onoff,zorder=$zorder,groupsid=$groupsid WHERE id=$rid");
