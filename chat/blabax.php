...
if($uid>6){
	...
	$res=neutral_query('SELECT * FROM '.$dbss['prfx'].'_rooms WHERE groupsid=' .$group);
	while($row=neutral_fetch_array($res)){
	//if($row['id']==$room_coosel && $room_getsel<1){$landing_room=$room_coosel;}
	//if($row['id']==$room_getsel){$landing_room=$room_getsel;}
	if($row['groupsid']==$group){$landing_room=$row['id'];}
...
neutral_query('INSERT INTO '.$dbss['prfx']."_messages VALUES(NULL,999999,0,'$group','$sys_xuser',0,'','$uname $msg_enter',0,0,$timestamp)");
}
