...
export default {
  ...
  methods: {
    ...
    showvalue() {
      ...
      alert(
        "Dies ist Ihre aktuelle Diagnose. Bitte nehmen Sie Änderungen vor, wenn Sie möchten. Fahren Sie trotzdem fort."
      );
    },
    ...
    sendvalue() {
      ...
      if (l.disabled) {
        data.append("labloop", 1);
      } else {
        data.append("labloop", l.checked);
      }
      data.append("onlineuser", localStorage.email);
      axios
        .post(
          // "./Api/api.php?action=login",
          "./Api/rigas_api.php?action=sendsubmit2",
          data
        )
        .then((res) => {
          if (res.data.error) {
    ...
    submitcounter() {
      ...
      if (b.checked == true) {
        ...
        axios
          .post(
            // "./Api/api.php?action=countervariable",
            "./Api/rigas_api.php?action=submitvariable2",
            data
          )
          .then((res) => {
            if (res.data.error) {
