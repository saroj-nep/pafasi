  ...
  <div class="grid justify-center">
    ...
    <div class="grid grid-cols-1 gap-2">
      <div>
        ...
        <Popup2
          v-if="popupTriggers.nichtsTrigger"
          :TogglePopup="() => TogglePopup('nichtsTrigger')"
        >
      ...
      <div>
        ...
        <Popup2
          v-if="popupTriggers.andernTrigger"
          :TogglePopup="() => TogglePopup('andernTrigger')"
        >
          <div
            class="tooltip"
...
import Submit3 from "./Holderbaum_Submit3.vue";
import Specialties from "./Holderbaum_Facharzt/Holderbaum_Specialties.vue";
import axios from "axios";

export default {
  created() {
    this.currentpage(), this.getdoctorloops();
  },
  methods: {
    currentpage() {
      ...
      data.append("afterspecialties", 1);
    ...
    getdoctorloops() {
      axios
        ...
        .then((response) => {
          this.doctorloops = response.data;
        });
    },
    goToEvents: function () {
      var data = new FormData();
      ...
      axios
        .post(
          // "./Api/api.php?action=countervariable",
          "./Api/holderbaum_api.php?action=submitdoctorloop",
          data
        )
        .then((res) => {
          if (res.data.error) {
