...
<button type="submit" style="display:block" id="submitbutton"  class="submitbutton btn shadow-[0_9px_0_rgb(0,0,0)] hover:shadow-[0_4px_0px_rgb(0,0,0)] text-black bg-white ease-out hover:translate-y-1 transition-all rounded shadow-xl"
      ...
      axios
        .post(
          // "./Api/api.php?action=login",
           "./Api/bauch_api.php?action=sendsubmit1",
          data
        )
        .then(res => {
          if (res.data.error) {
...
if (a.checked==1){ if (b.checked==1){data.append("economy",-5.88)} else {data.append("economy",-2.94)}}
else {if (b.checked==1){data.append("economy",-2.94)}}
...
if (a.checked==1){data.append("safety",100);
...
else if (b.checked==true){data.append("safety",95);
                     data.append("economy",5.88) 
                     }
else if (c.checked==true){data.append("safety",0);
                     data.append("economy",-2.94 ) 
                     }
else if (d.checked==true){data.append("safety",60);
                     data.append("economy",-2.94  ) 
                     }   
else if (e.checked==true){data.append("safety",50);
                     data.append("economy",-2.94  ) 
                     }
else if (f.checked==true){data.append("safety",50);
                     data.append("economy",-2.94 ) 
                     }
else if (g.checked==true){data.append("safety",0);
                     data.append("economy",-2.94 ) 
}
 data.append("step",'Sie haben Ihre Diagnose eingereicht');
 data.append("onlineuser",localStorage.email);
 ...
 axios
        .post(
          // "./Api/api.php?action=countervariable",
          "./Api/bauch_api.php?action=submitvariable",
          data
        )
        .then(res => {
          if (res.data.error) {
